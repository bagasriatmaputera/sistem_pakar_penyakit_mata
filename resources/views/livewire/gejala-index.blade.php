<div class="max-w-md mx-auto bg-slate-50 min-h-screen shadow-lg flex flex-col justify-between font-sans">
    <div>
        <div class="bg-gradient-to-b from-blue-700 to-sky-500 p-5 text-white flex items-center justify-between rounded-b-xl shadow-md">
            <div>
                <h1 class="text-lg font-bold tracking-wide">Basis Data Gejala</h1>
                <p class="text-[10px] text-blue-100 uppercase tracking-widest">Manajemen Aturan Sistem Pakar</p>
            </div>
            <a href="{{ route('admin.gejala.create') }}" class="bg-white text-blue-700 hover:bg-blue-50 text-xs font-bold px-3 py-2 rounded-xl shadow transition flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path></svg>
                Tambah
            </a>
        </div>

        <x-breadcrumbs :paths="['Admin' => '#', 'Gejala' => '#']" />

        <div class="p-4 space-y-4">
            <div class="relative">
                <input type="text" wire:model.live="search" placeholder="Cari kode atau nama gejala..." 
                ...
                <table class="w-full text-left border-collapse text-[11px]">
                    <thead>
                        <tr class="bg-blue-950 text-white uppercase tracking-wider font-bold">
                            ...
                            <th class="px-3 py-3">Nama Gejala</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($gejalas as $gejala)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-3 py-3 text-center font-black text-blue-900 bg-slate-50/50">
                                    {{ $gejala->kode_gejala }}
                                </td>
                                <td class="px-3 py-3">
                                    <div class="font-bold text-gray-800 leading-tight">{{ $gejala->nama_gejala }}</div>
                                    @if($gejala->deskripsi_gejala)
                                        <div class="text-[10px] text-gray-400 mt-0.5 line-clamp-1 text-justify">{{ $gejala->deskripsi_gejala }}</div>
                                    @endif
                                </td>
                                <td class="px-2 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('admin.gejala.edit', $gejala->id) }}" class="p-1 text-sky-600 bg-sky-50 hover:bg-sky-100 rounded-md transition" title="Ubah">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                        </a>
                                        <button type="button" wire:click="deleteGejala({{ $gejala->id }})" wire:confirm="Apakah Anda yakin ingin menghapus gejala ini secara permanen?" 
